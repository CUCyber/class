...
SOURCES!=find * \( -path '*.res' -o -path 'vendor' -o -path 'resources' -o -path '$(OUTDIR)' \) -prune -o -type f -name '*.md' -a -not \( -name 'LICENSE.md' -o -name 'README.md' \) -print
...
update: $(WEBSITE)$(SITE)$(ROOT)
	...
	git -C '$(WEBSITE)' commit -m 'update class'
	git -C '$(WEBSITE)' push
